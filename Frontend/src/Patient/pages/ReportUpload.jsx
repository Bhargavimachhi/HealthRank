...
import { Upload, FileText, Activity, Clock, Pill, ArrowLeft, Stethoscope } from "lucide-react"
...
export default function ReportUploadPage() {
  ...
  const reportTypes = [
    { value: "cbc", label: "Complete Blood Count Report", icon: "🩸" },
    { value: "creatinine", label: "Serum Creatinine Report", icon: "🫘" },
    { value: "blood-sugar", label: "Random Blood Sugar", icon: "🍯" },
    { value: "urine", label: "Urine Routine Micro Report", icon: "💧" },
    { value: "electrolytes", label: "Serum Electrolytes Report", icon: "⚡" },
  ]
  ...
  const handleMedicationChange = (medication, checked) => {
    setFormData((prev) => ({
      ...prev,
      medications: checked ? [...prev.medications, medication] : prev.medications.filter((m) => m !== medication),
    }))
  }
        ...
        <Card className="shadow-xl border-0">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-xl">Medical Report & Symptoms</CardTitle>
                <CardDescription>
                  Step {currentStep} of 2 - {currentStep === 1 ? "Upload Report" : "Describe Symptoms"}
                </CardDescription>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      {reportTypes.map((type) => (
                        <Card
                          ...
                          className={`cursor-pointer transition-all hover:shadow-md ${
                            formData.reportType === type.value ? "ring-2 ring-blue-500 bg-blue-50" : "hover:bg-gray-50"
                          }`}
                    ...
                    {formData.reportFile && (
                      <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-green-800 font-medium">{formData.reportFile.name}</span>
              ...
              {currentStep === 2 && (
                <div className="space-y-6">
                  ...
                  {/* Pain Scale */}
                  <div className="space-y-4">
                    <h3 className="text-lg font-semibold">Pain Level (Optional)</h3>
                    <div className="space-y-4">
                      <Label>Rate your pain level from 1 (no pain) to 10 (severe pain)</Label>
                      <div className="px-4">
                        <Slider
                          value={formData.painScale}
                          onValueChange={(value) => setFormData((prev) => ({ ...prev, painScale: value }))}
                          max={10}
                          min={1}
                          step={1}
                          className="w-full"
                        ...
                        <div className="flex justify-between text-sm text-gray-500 mt-2">
                          <span>1 - No pain</span>
                          <span className="font-medium text-blue-600">Current: {formData.painScale[0]}</span>
                  ...
                  <div className="space-y-4">
                    ...
                    {formData.medications.length > 0 && (
                      <div className="bg-blue-50 p-3 rounded-lg">
                        <p className="text-sm text-blue-800">Selected: {formData.medications.join(", ")}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="flex gap-4 pt-6">
                {currentStep === 1 ? (
                  <>
                    <Button type="button" variant="outline" className="flex-1" onClick={() => router.push("/homepage")}>
                      Cancel
                    </Button>
